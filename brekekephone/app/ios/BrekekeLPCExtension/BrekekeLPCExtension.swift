...
class BrekekeLPCExtension: NEAppPushProvider {
  /// iOS can spin up a new provider in the SAME process (e.g. after
  /// nesessionmanager is killed under memory pressure and relaunched)
  /// WITHOUT calling stop() on the old one. The old object's BaseChannel
  /// keeps its LPC connection alive, so the new provider opens a SECOND
  /// connection with the same DeviceID -> the server kicks one -> the
  /// no-backoff reconnect loop becomes a connection storm.
  /// Keep a process-wide reference to the active channel and disconnect
  /// it when a new provider is created, so there is never more than one
  /// live LPC connection per process.
  ...
  override init() {
    ...
    // process before wiring up this one, so two connections with the same
    // DeviceID never overlap
