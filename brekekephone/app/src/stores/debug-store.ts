...
import { debounce, orderBy } from '@rntwsc/shared/lodash'
...
export class DebugStore {
  ...
  writeFile = async () => {
    if (this.loading) {
      BackgroundTimer.setTimeout(this.writeFileBatch, defaultTimeout)
      return
    }
    this.loading = true
  ...
  writeFileBatch = debounce(this.writeFile, defaultTimeout, {
    maxWait: 1000,
  })
