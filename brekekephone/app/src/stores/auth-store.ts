...
import type { CallHistoryInfo } from '@/stores/add-call-history'
...
import { clearUrlParams } from '@/utils/deeplink.native'
import type { UrlParams } from '@/utils/deeplink-parse'
import { deviceDetail, devicePlatform } from '@/utils/device'
import type { ParsedPn, SipPn } from '@/utils/push-notification-parse'
import { toBoolean } from '@/utils/string'
import { waitForActiveAppState } from '@/utils/wait-for-active-app-state'
...
export class AuthStore {
  ...
  private pendingCustomPageEvents: PendingCustomPageEvent[] = []
  ...
  setCustomPages = (pages: PbxCustomPage[]) => {
  ...
  private removeExpiredCustomPageEvents = () => {
    const now = Date.now()
  ...
  private markCustomPageEventPageCompleted = (
    eventKey: string,
    pageId: string,
  ) => {
  ...
  private navigateForCustomPageEvent = (pages: PbxCustomPage[]) => {
    if (!pages.length) {
      return true
    }
  ...
  private completeCustomPageEvent = (eventKey: string) => {
    this.pendingCustomPageEvents = this.pendingCustomPageEvents.filter(
  ...
  private resetPrevAccountConnection = () => {
    this.signedInId = ''
