...
export const PageChatDetail = observer(({ buddy }: { buddy: string }) => {
    ...
    await ctx.uc
      ...
      .catch((err: Error) => {
        ...
        if (ctx.auth.ucState !== 'success') {
          console.log('UC debug: getBuddyChats skipped, UC not signed in yet')
          return
        }
        RnAlert.error({
  ...
  // close the emoji panel once the keyboard has fully risen, handing the bottom
  // region back to it. rising-edge only: dismissing the keyboard to open the
  // panel is a falling edge and must not close it.
  ...
  return (
    <Layout
      ...
      bottomPanel={emojiTurnOn ? <EmojiPicker onSelect={insertEmoji} /> : null}
