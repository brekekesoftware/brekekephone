...
- Bug fix: Calls from a notification now connect reliably, even if the app was closed.
- Bug fix: An ongoing call is no longer interrupted by another account's call.
- Improvement: The camera stops while the app is in the background.
- Bug fix: Video now resumes correctly after hold.
- Improvement: Fallback local connection (LPC), the local push, is now a separate option under Push Notification, off by default.
- Bug fix: Wrong sign-in details now show an error instead of connecting forever.
...
- Implement open custom page from url scheme brekekephone://custompage?id=
- Implement webphone.resource-line.pattern pbx settings match
