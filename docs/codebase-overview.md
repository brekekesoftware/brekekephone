...
> Scope: current `master`, app version `3.0.0`. Version 3.0.0 was a full tooling/structure migration (see `CHANGELOG.md`): the repo moved from a single-package yarn/CRA-Craco layout to a pnpm monorepo (`brekekephone/app` for React Native, `brekekephone/web` for the Vite web build), file names moved from camelCase to kebab-case, MobX moved from legacy decorators to `makeAutoObservable`, and Old Architecture/Hermes were switched on instead of off. If you are reading old notes, PRs, or comments that predate this migration, treat file paths and some of the invariants in these docs as superseding them.

#### Quick summary

Brekeke Phone is a VoIP/SIP softphone running on Web, iOS, and Android. Many places look a bit "odd" not because the code was designed to look nice, but because the app has to keep a call alive across foreground, background, lock screen, killed app, or when woken up from a push notification.

The areas most prone to regressions are login, push notifications, CallKeep/CallKit, SIP/WebRTC, PBX/PAL, UC, MFA, LPC, multi-account, and custom page. If you touch one of these areas, test with real flows, not just lint/build. The codebase has inline `BUG-XXXX` comments (e.g. `BUG-1207`, `BUG-1238`, `BUG-1250`) marking spots that guard a specific past regression -- if you are about to change code near one, read the comment and understand what it prevents before touching it.
