...
- [Behavior](#behavior)
- [Triggering from a web page](#triggering-from-a-web-page)
...
- If there is an active call in progress, the URL scheme is ignored to avoid disrupting the call UI -- this case deliberately does not mark the link as "already handled", so the same link can be retried once the call ends
- If MFA is in progress for the account, the app waits for it to complete before opening the custom page

See [`handleDeepLinkCustomPage`](../brekekephone/app/src/stores/auth-store.ts) for the exact current implementation, and [Custom page](./custom-page.md) for how the custom page itself loads/reloads once opened.
