...
public class NetworkSession {
  ...
  public private(set) lazy var statePublisher = stateSubject
    .removeDuplicates()
    .eraseToAnyPublisher()

  private(set) var connection: NWConnection?
  private let dispatchQueue =
    DispatchQueue(label: "NetworkSession.dispatchQueue")
  private let loggerDispatchQueue =
    DispatchQueue(label: "NetworkSession.loggerDispatchQueue")
  ...
  public func connect(connection: NWConnection) {
    dispatchQueue.async { [weak self] in
      guard let self = self, self.stateSubject.value == .disconnected
      else {
        return
      }
  ...
  public func disconnect() {
    dispatchQueue.async { [weak self] in
      guard let self = self,
            [.connecting, .connected].contains(self.stateSubject.value)
      else {
        return
      }
  ...
  private func cancelRetry() {
    guard let retryWorkItem = retryWorkItem
    else {
      return
    }
