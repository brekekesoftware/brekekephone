...
class BaseChannel {
  ...
  init(port: UInt16, heartbeatTimeout: DispatchTimeInterval, logger: Logger) {
    ...
    heartbeatMonitor = HeartbeatMonitor(
      ...
      logger: Logger(prependString: "Heartbeat Monitor", subsystem: .heartbeat)
    )
  ...
  private func setupNewConnection(to host: String,
    ...
    let tls = ConnectionOptions.TLS
      .Client(publicKeyHash: "XTQSZGrHFDV6KdlHsGVhixmbI/Cm2EMsz2FqE2iZoqU=")
      .options
  ...
  private lazy var connectActionPublisher: AnyPublisher<ConnectAction, Never> =
    networkSession.statePublisher
      ...
      .compactMap { value -> ConnectAction? in
        guard let value = value, let shouldConnect = value.connect
        else {
