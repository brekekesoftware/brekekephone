...
public class FIRMessagingModule extends ReactContextBaseJavaModule
  ...
  public FIRMessagingModule(ReactApplicationContext reactContext) {
    ...
    mFIRLocalMessagingHelper =
        new FIRLocalMessagingHelper((Application) reactContext.getApplicationContext());
  ...
  public void createNotificationChannel(ReadableMap details, Promise promise) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      NotificationManager mngr =
          (NotificationManager) getReactApplicationContext().getSystemService(NOTIFICATION_SERVICE);
  ...
  private void registerTokenRefreshHandler() {
    IntentFilter intentFilter = new IntentFilter("com.brekeke.fcm.FCMRefreshToken");
  ...
  private String getStringFromReadableMap(ReadableMap map, String key) throws Exception {
    switch (map.getType(key)) {
      ...
      default:
        throw new Exception(
            "Unknown data type: " + map.getType(key).name() + " for message key " + key);
    }
  }

  private void registerMessageHandler() {
    IntentFilter intentFilter = new IntentFilter("com.brekeke.fcm.ReceiveNotification");
  ...
  private void registerLocalMessageHandler() {
    IntentFilter intentFilter = new IntentFilter("com.brekeke.fcm.ReceiveLocalNotification");
