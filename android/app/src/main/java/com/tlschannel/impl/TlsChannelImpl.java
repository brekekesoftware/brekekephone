...
public class TlsChannelImpl implements ByteChannel {
    ...
    try {
      ...
      if (logger.isLoggable(Level.FINEST)) {
        logger.log(
            ...
            new Object[] {
              Util.resultToString(result), engine.getHandshakeStatus(), inEncrypted, dest
            });
      }
      return result;
  ...
  public static void callChannelRead(ReadableByteChannel readChannel, ByteBuffer buffer)
    ...
    logger.log(Level.INFO, "Read from channel; response: {}, buffer: {}", new Object[] {c, buffer});
    if (c == -1) {
  ...
  private SSLEngineResult callEngineWrap(ByteBufferSet source) throws SSLException {
    try {
      ...
      if (logger.isLoggable(Level.INFO)) {
        ...
        logger.log(
            Level.INFO, "engine.wrap() result: [{0}]", new Object[] {Util.resultToString(result)});
      }
      return result;
    } catch (SSLException e) {
      invalid = true;
    ...
    try {
      ...
      if (force || !negotiated) {
        ...
        try {
          ...
          logger.log(
              Level.FINEST, "client code threw exception in session initialization callback", e);
      ...
      try {
        if (!shutdownSent) {
          try {
            ...
            logger.log(
                Level.FINEST,
                "error doing TLS shutdown on close(), continuing: {0}",
                e.getMessage());
          }
        }
      } finally {
