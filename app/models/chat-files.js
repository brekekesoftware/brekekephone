...
const allowedToCreateProps = [
  'id', 'name', 'size', 'incoming',
  'transferPercent', 'transferWaiting', 'transferStarted',
  'transferSuccess', 'transferStopped', 'transferFailure'
]
...
export default createModel({
  ...
  action: {
    create: (prevState, file) => immutable.on(prevState)(
      immutable.vset(`byId.${file.id}`, validateCreatingFile(file))
    ),
    update: (prevState, file) => immutable.on(prevState)(
      immutable.fset(`byId.${file.id}`, (old) => ({...old, ...validateUpdatingFile(file)}))
    ),
