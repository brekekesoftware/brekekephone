...
export default createModel({
  ...
  action: {
    ...
    appendByBuddy: (s, buddy, chats) => immutable.on(s)(
      ...
      immutable.fset(`idsMapByBuddy.${buddy}`, (ids = []) => [...ids, ...mapChatsToIds(chats)])
    ),
    prependByBuddy: (s, buddy, chats) => immutable.on(s)(
      immutable.fset('buddyIdsByRecent', (ids) => uniqArray([buddy, ...ids])),
      immutable.fset('detailMapById', (map) => ({...map, ...reduceChatsToMapById(chats.map(validateChat))})),
      immutable.fset(`idsMapByBuddy.${buddy}`, (ids = []) => [...mapChatsToIds(chats), ...ids])
    )
  }
})
