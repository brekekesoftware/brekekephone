...
const validateCreatingGroup = (group) => ({...propDefault, ...pickProps(group, allowedToCreateProps)})
...
export default createModel({
  ...
  action: {
    ...
    create: (prevState, group) => immutable.on(prevState)(
      immutable.fset('idsByOrder', (idsByOrder) => uniqArray([group.id, ...idsByOrder])),
      immutable.vset(`detailMapById.${group.id}`, validateCreatingGroup(group))
    ),
    update: (prevState, group) => immutable.on(prevState)(
      immutable.fset(`detailMapById.${group.id}`, (old) => ({...old, ...validateUpdatingGroup(group)}))
    ),
    remove: (prevState, id) => immutable.on(prevState)(
      immutable.fset('idsByOrder', (idsByOrder) => idsByOrder.filter((_) => _ !== id)),
