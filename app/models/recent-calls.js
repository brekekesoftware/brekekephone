...
const allowedToCreateProps = ['profile', 'id', 'incoming', 'answered', 'partyName', 'partyNumber', 'created']
...
export default createModel({
  ...
  action: {
    create: (prevState, call) => immutable.on(prevState)(
      immutable.fset(`idsMapByProfile.${call.profile}`, (ids = []) => [call.id, ...ids]),
      immutable.vset(`detailMapById.${call.id}`, validateCreatingCall(call))
    ),
    remove: (prevState, id) => immutable.on(prevState)(
      immutable.fset(`idsMapByProfile.${prevState.detailMapById[id] && prevState.detailMapById[id].profile}`, (ids = []) => ids.filter((_id) => _id !== id)),
