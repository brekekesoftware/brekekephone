...
const allowedToCreateFields = [
  'id', 'pbxHostname', 'pbxPort', 'pbxTenant', 'pbxUsername',
  'pbxPassword', 'parks', 'ucEnabled', 'ucHostname', 'ucPort'
]
const validateCreatingData = (data) => pick(data, allowedToCreateFields)

const allowedToUpdateFields = [
  'pbxHostname', 'pbxPort', 'pbxTenant', 'pbxUsername',
  'pbxPassword', 'parks', 'ucEnabled', 'ucHostname', 'ucPort'
]
...
export default createModel({
  ...
  action: {
    create: (prevState, profile) => immutable.on(prevState)(
      immutable.fset('idsByOrder', (ids) => [...ids, profile.id]),
      immutable.vset(`detailMapById.${profile.id}`, validateCreatingData(profile))
    ),
    update: (prevState, profile) => immutable.on(prevState)(
      immutable.fset(`detailMapById.${profile.id}`, (old) => ({...old, ...validateUpdatingData(profile)}))
    ),
