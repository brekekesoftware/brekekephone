...
const allowedToCreateProps = [
    'id', 'incoming', 'partyName', 'partyNumber',
    'localVideoEnabled', 'remoteVideoStreamURL', 'remoteVideoStreamObject', 'createdAt'
]
...
export default createModel({
    ...
    action: {
        ...
        update: function(state, call) {
            const obj = immutable.on(state)(
                immutable.fset(`detailMapById.${call.id}`, (old) => ({...old, ...validateUpdatingCall(call)}))
            );
            return obj;
        },
