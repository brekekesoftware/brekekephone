...
const allowedToCreateProps = ['id', 'name', 'avatar', 'offline', 'online', 'idle', 'busy', 'mood']
const validateCreatingUser = (user) => pickProps(user, allowedToCreateProps)

const allowedToUpdateProps = ['name', 'avatar', 'offline', 'online', 'idle', 'busy', 'mood']
...
export default createModel({
  ...
  action: {
    ...
    update: (prevState, user) => immutable.on(prevState)(
      immutable.fset(`detailMapById.${user.id}`, (old) => ({...old, ...validateUpdatingUser(user)}))
    )
  }
})
