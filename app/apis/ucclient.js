...
(function (root, factory) {
    ...
    ChatClient.prototype = {
        ...
        signIn: function(host, path, tenant, user, pass, option, funcOK, funcError) {
            ...
            if (host && !user && !pass && !option && !funcOK && !funcError) { // signIn(option, funcOK, funcError)
                funcError = tenant;
                funcOK = path;
                option = host;
                pass = option.pass;
                ...
                if (option.url) {
                    var url = string(option.url).replace(trimRegExp, "").replace(/\/$/, "").split("/");
                    path = url.pop();
                    host = url.join("/");
                } else {
                    path = option.path;
            ...
            if (option.usePhone) {
                ...
                if (!window.webkitRTCPeerConnection && !window.mozRTCPeerConnection && !window.RTCPeerConnection) {
                    if (funcError) {
                        var ev = {
            ...
            this._host = string(host).replace(trimRegExp, "").replace(/\/$/, "").split("/").pop();
            ...
            this._nonbuddylist = {};
            this._configProperties = {};
            this._status = !option.status && option.status !== 0 ? null : int(option.status);
            ...
            this._signInTimer = setTimeout(this._byThis(this._signInTimeout), int(option.timeout) || this.SIGN_IN_TIMEOUT_DEFAULT);
        ...
        loadPhoneProperties: function(options, funcOK, funcError) {
            ...
            this._rpcCall("GetPhoneProperties",
                {
                    tenant: this._tenant,
        ...
        saveProperties: function(profile, settings, buddylist, funcOK, funcError) {
            ...
            if (!settings) {
                ...
                if (settings.optional_settings && this._settings.optional_settings &&
                    settings.optional_settings.display_name !== this._settings.optional_settings.display_name) {
                ...
                function() {
                    this._rpcCall("GetProperties",
                        ...
                        function(result) {
                            ...
                            if (this.getConfigProperties().buddy_mode !== Constants.BUDDY_MODE_AUTO) {
                                this._buddylist = result.buddylist;
                            }
                            // remove nonbuddy
                            if (this._buddylist && this._buddylist.user && this._buddylist.user.length &&
                                this._nonbuddylist && this._nonbuddylist.user && this._nonbuddylist.user.length) {
                                for (var i = 0; i < this._buddylist.user.length; i++) {
                                    for (var j = this._nonbuddylist.user.length - 1; j >= 0; j--) {
                                        if (this._buddylist.user[i].tenant === this._nonbuddylist.user[j].tenant &&
                                            this._buddylist.user[i].user_id === this._nonbuddylist.user[j].user_id) {
                            ...
                            if (mustReSubscribe) {
                                ...
                                if (this._nonbuddylist && this._nonbuddylist.user && this._nonbuddylist.user.length) {
        ...
        uploadProfileImage: function(input, funcOK, funcError) {
            ...
            if (this._upload_ids[key]) { // already prepared (re-upload)
                this.cancelProfileImage({ tenant: tenant, user_id: user_id });
            }
            
            this._rpcCall("PrepareProfileImage",
                {
                    tenant: tenant,
                    user_id: user_id
                },
                function(result) {
                    ...
                    var checkFunction = function() {
                        this._rpcCall("CheckProfileImage",
                            {
                                tenant: tenant,
                                user_id: user_id,
                                upload_id: upload_id
                            ...
                            function(result) {
                                if (result.code === 0) { // Upload completed
                                    this._upload_ids[key] = upload_id;
                                    if (funcOK) {
                                        var ev = {
                                            url: (this._useHttps ? "https://" : "http://") + this._host + "/" + this._path + "/image?ACTION=DOWNLOAD&DOWNLOAD_ID=" + encodeURIComponent(result.download_id)
                                        };
                                        funcOK(ev);
                                    }
                                } else if (result.code === 1) { // Now uploading
                                    if (new Date().getTime() > startTime + this.UPLOAD_TIMEOUT_DEFAULT) { // Upload timeout
                                        this._rpcCall("CancelProfileImage", { tenant: tenant, user_id: user_id, upload_id: upload_id }, null, null);
                                        if (funcError) {
                                            var ev = {
                    ...
                    if (window.FormData) { // FormData enabled
                        ...
                        xhr.open("POST", (this._useHttps ? "https://" : "http://") + this._host + "/" + this._path + "/image?ACTION=UPLOAD&UPLOAD_ID=" + encodeURIComponent(upload_id) + "&tenant=" + encodeURIComponent(tenant) + "&user=" + encodeURIComponent(user_id), true);
                        xhr.onload = this._byThis(checkFunction);
                        xhr.send(fd);
                    } else {
                        // upload with form
                        input.form.action = (this._useHttps ? "https://" : "http://") + this._host + "/" + this._path + "/image?ACTION=UPLOAD&UPLOAD_ID=" + encodeURIComponent(upload_id) + "&tenant=" + encodeURIComponent(tenant) + "&user=" + encodeURIComponent(user_id);
                        input.form.method = "POST";
                        input.form.enctype = "multipart/form-data";
                        input.form.submit();
                        setTimeout(this._byThis(checkFunction), 1000);
        ...
        cancelProfileImage: function(options) {
            ...
            if (options && options.user_id) {
                keyToCancel = JSON.stringify({ tenant: options.tenant || this._tenant, user_id: options.user_id });
            }
        ...
        saveProfileImage: function(options, funcOK, funcError) {
            if (typeof options === "function" && typeof funcOK === "function" && !funcError) {
                funcError = funcOK;
                funcOK = options;
                options = {};
            }
            ...
            var upload_id = this._upload_ids[key];
            if (upload_id) {
                this._rpcCall("SaveProfileImage",
                    {
                        tenant: tenant,
                        user_id: user_id,
                        upload_id: upload_id
        ...
        deleteProfileImage: function(options, funcOK, funcError) {
            ...
            this._rpcCall("DeleteProfileImage",
                {
                    tenant: tenant,
                    user_id: user_id
                },
            ...
            for (var i = 0; i < buddyList.length; i++) {
                ...
                if (this._buddylist && this._buddylist.user && this._buddylist.user.length) {
                    for (var j = 0; j < this._buddylist.user.length; j++) {
                        if (this._buddylist.user[j].user_id === user_id && (this._buddylist.user[j].tenant || this._tenant) === tenant) {
                            // already buddy
                            exists = true;
                            break;
                        }
                ...
                if (this._nonbuddylist && this._nonbuddylist.user && this._nonbuddylist.user.length) {
                    for (var j = 0; j < this._nonbuddylist.user.length; j++) {
                        if (this._nonbuddylist.user[j].user_id === user_id && (this._nonbuddylist.user[j].tenant || this._tenant) === tenant) {
                            // already nonbuddy
                            exists = true;
                            break;
                        }
        ...
        removeTemporaryBuddy: function(buddyList, funcOK, funcError) {
            ...
            // unsubscribe
            if (users.length > 0) {
                this._rpcCall("UnsubscribeStatus",
                    {
                        users: users
                    },
                    function(result) {
        ...
        changeStatus: function(status, display, funcOK, funcError) {
            ...
            // notify status
            this._sendStatus(funcOK, this._byThis(function(error) {
                ...
                if (isNaN(action_id) || isNaN(month)) {
                    if (funcError) {
                        var ev = {
                            ...
                            message: "Invalid value: received_text_id_array[" + i + "] = " + received_text_id_array[i]
                        };
                        funcError(ev);
                    }
                    return;
        ...
        receiveUnreceivedConferenceText: function(options, funcOK, funcError) {
            ...
            var params = {};
            if (options && options.conf_id) { params.conf_id = string(options && options.conf_id); }
            if (options && options.conf_status) { params.conf_status = int(options && options.conf_status); }
            
            this._rpcCall("GetUnreceivedConferenceText",
                params,
                function(result) {
                    var messages = [];
                    if (result && result.message && result.message.length) {
        ...
        createConference: function(subject, invite, funcOK, funcError) {
            ...
            this._rpcCall("CreateConference",
                {
        ...
        inviteToConference: function(conf_id, invite, funcOK, funcError) {
            ...
            this._rpcCall("InviteToConference",
                {
        ...
        sendFile: function(target, input, funcOK, funcError) {
            ...
            var size = 0;
            var ext = "";
            if (input.files) { // File API enabled
                ...
                function(result) {
                    ...
                    this._rpcCall("SendText", sendTextParams,
                        function(result) {
                            ...
                            // new file info
                            this._newFileInfo(file_id, target, true, Constants.FILE_STATUS_UNACCEPTED, name, size, input.form, fd);
        ...
        sendFiles: function(options, fileList, funcOK, funcError, evRecursion) {
            ...
            if (size === 0) { // folder
                ...
                // new file info
                this._newFileInfo(file_id, target, true, Constants.FILE_STATUS_ERROR, name, size, null, null);
        ...
        acceptFile: function(file_id, form, funcError) {
            ...
            // start download post
            form.action = (this._useHttps ? "https://" : "http://") + this._host + "/" + this._path + "/file?ACTION=DOWNLOAD&SUBACTION=TRANS&TRANSFER_ID=" + encodeURIComponent(file_id) + "&tenant=" + encodeURIComponent(this._tenant) + "&user=" + encodeURIComponent(this._user_id);
            form.method = "POST";
        ...
        cancelFile: function(file_id, funcError) {
            ...
            if (fileInfo.isUpload) { // cancel upload
                this._rpcCall("SendClientEvent",
                    ...
                    function(result) {
                        this._rpcCall("StopFileTransfer",
                            {
                                file_id: file_id
                            },
                            function(result) {
        ...
        makeCall: function(target, mediaConstraints, option, funcOK, funcError) {
            ...
            if (!mediaConstraints || !(mediaConstraints.audio || mediaConstraints.video)) {
                if (funcError) {
                    var ev = {
            ...
            if (!option.call_id) {
                ...
                if (!this._sessionBundleTable[sessionBundleId] || this._makeCallStatus[sessionBundleId]) {
                    if (funcError) {
                        var ev = {
            ...
            if (target.user_id && option.sharedObject) {
                ...
                this._sendSharedObject(target, option.sharedObject, this._byThis(this.makeCall, [target, mediaConstraints, newOption, funcOK, funcError]), funcError);
                return;
            }
            var sharedObjectJson = JSON.stringify(option.sharedObject || option._sharedObject || {});
            ...
            if (target.user_id) {
                ...
                if (this._phoneProperties.mode_uc_call === 1 || (this._phoneProperties.mode_uc_call === 2 && mediaConstraints.video)) {
                    var buddyPhone = this._buddyPhone[JSON.stringify({ tenant: target.tenant, user_id: target.user_id })];
                    if (buddyPhone) {
                ...
                if (!this._conferences[conf_id] || int(this._conferences[conf_id].conf_status) !== Constants.CONF_STATUS_JOINED) {
                    if (funcError) {
                        var ev = {
                ...
                if (this._phoneProperties.mode_uc_call === 1 || (this._phoneProperties.mode_uc_call === 2 && mediaConstraints.video)) {
                    for (var i = 0; i < this._conferences[conf_id].user.length; i++) {
                        ...
                        if (int(user.video_conf_status) === 7) {
                            var buddyPhone = this._buddyPhone[JSON.stringify({ tenant: string(user.tenant || this._tenant), user_id: string(user.user_id) })];
                            if (buddyPhone) {
                                targetUsers.push({
            ...
            if (target.conf_id && makeCallParamsArray.length === 0) {
                ...
                if (!this._sessionBundleTable[sessionBundleId]) {
                    ...
                    this._sessionBundleTable[sessionBundleId].lastCallInfo = JSON.stringify(callInfo);
                }
                if (funcOK) {
                    var ev = {
                        callInfo: callInfo
            ...
            var timer = setTimeout(this._byThis(this._makeCallTimeout, [sessionBundleId]), this.MAKE_CALL_TIMEOUT_DEFAULT);
        ...
        muteMicrophone: function(call_id, muted) {
            ...
            if (sessionBundle) {
                ...
                for (var i = 0; i < sessionBundle.sessionIdArray.length; i++) {
                    var session = this._phone.getCallSession(sessionBundle.sessionIdArray[i]);
            ...
            if (sessionBundle) {
                ...
                for (var i = 0; i < sessionBundle.sessionIdArray.length; i++) {
                    ...
                    if (session && session.status === "incoming") {
                        var audioConstraints = session.mediaConstraints.audio ? sessionBundle.mediaConstraints.audio : false;
                        var videoConstraints = session.mediaConstraints.video ? sessionBundle.mediaConstraints.video : false;
        ...
        sendDTMF: function(call_id, tone) {
            ...
            if (sessionBundle) {
                for (var i = 0; i < sessionBundle.sessionIdArray.length; i++) {
                    var session = this._phone.getCallSession(sessionBundle.sessionIdArray[i]);
                    if (session && session.status === "answered" && session.mediaConstraints.audio) {
        ...
        searchTopicsByDate: function(date, funcOK, funcError) {
            ...
            var dateString = date.getFullYear() + "-" + ("0" + (date.getMonth() + 1)).slice(-2) + "-" + ("0" + date.getDate()).slice(-2) + " 00:00:00";
            
            this._rpcCall("LogSearch",
                {
                    date: dateString
                },
                function(result) {
                    ...
                    for (var i = 0; i < result.topics.length; i++) {
                        if (result.topics[i].conf_id) {
                            ...
                            var opposite;
                            if (result.topics[i].sender.tenant !== this._tenant || result.topics[i].sender.user_id !== this._user_id) {
                                opposite = result.topics[i].sender;
                            } else {
                                opposite = result.topics[i].receiver;
                            }
                            if (opposite) {
                                if (addedUsers.indexOf(JSON.stringify({ tenant: opposite.tenant, user_id: opposite.user_id })) === -1) {
                                    var topic = {
                                        topic_id: string(++this._topicIdCounter),
                                    ...
                                    topics.push(topic);
                                    addedUsers.push(JSON.stringify({ tenant: opposite.tenant, user_id: opposite.user_id }));
                                    this._topics[topic.topic_id] = {
                                        date: dateString,
        ...
        searchLogsByTopic: function(topic_id, funcOK, funcError) {
            ...
            this._rpcCall("GetLog", this._topics[topic_id],
                function(result) {
                    var logs = [];
        ...
        searchTopicTexts: function(topic_id, funcOK, funcError) {
            ...
            this._rpcCall("SearchTopicTexts",
                {
        ...
        loadSystemProperties: function(funcOK, funcError) {
            ...
            this._rpcCall("GetSystemProperties",
                {
                    type: "all"
                },
                function(result) {
        ...
        saveSystemProperties: function(systemProperties, funcOK, funcError) {
            ...
            this._rpcCall("SetSystemProperties", params,
                function(result) {
                    if (funcOK) {
                        var ev = {
                        };
        ...
        loadTenantProperties: function(funcOK, funcError) {
            ...
            if (this.getProfile().user_type !== Constants.USER_TYPE_SYSTEM_ADMIN && this.getProfile().user_type !== Constants.USER_TYPE_TENANT_ADMIN) {
                if (funcError) {
                    var ev = {
                        code: Errors.INVALID_USER_TYPE,
                        message: "Invalid user_type"
            ...
            this._rpcCall("GetTenantProperties",
                {
                },
                function(result) {
                    if (funcOK) {
        ...
        saveTenantProperties: function(tenantProperties, funcOK, funcError) {
            ...
            this._rpcCall("SetTenantProperties", tenantProperties,
                function(result) {
                    if (funcOK) {
                        var ev = {
                        };
        ...
        createTenantProperties: function(tenant, funcOK, funcError) {
            ...
            this._rpcCall("CreateTenantProperties",
                {
        ...
        createTenantListFromPbx: function(funcOK, funcError) {
            ...
            this._rpcCall("CreateTenantListFromPbx",
                {
                },
                function(result) {
                    if (funcOK) {
        ...
        loadTenantSettings: function(funcOK, funcError) {
            ...
            this._rpcCall("GetTenantSettings",
                {
                },
                function(result) {
                    if (funcOK) {
        ...
        saveTenantSettings: function(tenantSettings, funcOK, funcError) {
            ...
            this._rpcCall("SetTenantSettings", tenantSettings,
                function(result) {
                    if (funcOK) {
                        var ev = {
                        };
        ...
        loadTenantListFromPbx: function(funcOK, funcError) {
            ...
            this._rpcCall("GetTenantListFromPbx",
                {
                },
                function(result) {
                    if (funcOK) {
        ...
        loadAdvancedSettings: function(funcOK, funcError) {
            ...
            this._rpcCall("GetSystemProperties",
                ...
                function(result) {
                    if (funcOK) {
                        var ev = {
                            advancedSettings: string(result && result.adv && result.adv.settings)
                        };
                        funcOK(ev);
                    }
                },
        ...
        getProfile: function() {
            ...
            profile.profile_image_url = this._getProfileImageUrl(this._tenant, this._user_id);
            return profile;
        },
        ...
        getSettings: function() {
            ...
            settings.buddylist_settings.display_type = this._settings && this._settings.buddylist_settings ? int(this._settings.buddylist_settings.display_type) : 0;
            settings.initial_status = this._settings ? int(this._settings.initial_status) : 0;
            ...
            if (this._buddylist && this._buddylist.user && this._buddylist.user.length) {
                for (var i = 0; i < this._buddylist.user.length; i++) {
                    ...
                    if (buddy.user_id || buddy.user_id === "") {
                        ...
                        userBuddy.profile_image_url = this._getProfileImageUrl(userBuddy.tenant, userBuddy.user_id);
        ...
        getBuddyUser: function(buddy) {
            ...
            if (this._buddylist && this._buddylist.user && this._buddylist.user.length) {
                for (var i = 0; i < this._buddylist.user.length; i++) {
                    if (this._buddylist.user[i].user_id === user_id && (this._buddylist.user[i].tenant || this._tenant) === tenant) {
                        userBuddy.user_id = string(this._buddylist.user[i].user_id);
                        userBuddy.tenant = string(this._buddylist.user[i].tenant || this._tenant);
            ...
            if (this._nonbuddylist && this._nonbuddylist.user && this._nonbuddylist.user.length) {
                for (var i = 0; i < this._nonbuddylist.user.length; i++) {
                    if (this._nonbuddylist.user[i].user_id === user_id && (this._nonbuddylist.user[i].tenant || this._tenant) === tenant) {
                        userBuddy.user_id = string(this._nonbuddylist.user[i].user_id);
                        userBuddy.tenant = string(this._nonbuddylist.user[i].tenant || this._tenant);
        ...
        getConfigProperties: function() {
            ...
            configProperties.buddy_mode = this._configProperties ? int(this._configProperties.buddy_mode) : 0;
            configProperties.chat_mode = this._configProperties ? int(this._configProperties.chat_mode) : 0;
            configProperties.webnotif_timeout = this._configProperties ? int(this._configProperties.webnotif_timeout) : 0;
            configProperties.webchat_enabled = this._configProperties ? string(this._configProperties.webchat_enabled).toLowerCase() : "false";
            if (this._configProperties && this._configProperties.optional_config) {
                configProperties.optional_config = JSON.parse(JSON.stringify(this._configProperties.optional_config));
            }
        ...
        getBuddyStatus: function(buddy) {
            var status = {};
            var buddyStatus = this._buddyStatus[JSON.stringify({ tenant: buddy.tenant || this._tenant, user_id: buddy.user_id })];
            status.status = buddyStatus ? int(buddyStatus.status) : Constants.STATUS_OFFLINE;
        ...
        getConference: function(conf_id) {
            ...
            if (this._conferences[conf_id]) {
                ...
                if (this._conferences[conf_id].from) {
                    ...
                    conference.from.user_id = string(this._conferences[conf_id].from.user_id);
                    conference.from.user_name = string(this._conferences[conf_id].from.user_name);
                }
                if (this._conferences[conf_id].creator) {
                    conference.creator.tenant = string(this._conferences[conf_id].creator.tenant);
                    conference.creator.user_id = string(this._conferences[conf_id].creator.user_id);
                    conference.creator.user_name = string(this._conferences[conf_id].creator.user_name);
                }
                ...
                if (this._conferences[conf_id].invite_properties) {
                    conference.invite_properties.invisible = Boolean(this._conferences[conf_id].invite_properties.invisible);
                    conference.invite_properties.webchatfromguest = this._conferences[conf_id].invite_properties.webchatfromguest ? JSON.parse(JSON.stringify(this._conferences[conf_id].invite_properties.webchatfromguest)) : null;
                }
                for (var i = 0; i < this._conferences[conf_id].user.length; i++) {
                    var user = {};
                    user.user_id = string(this._conferences[conf_id].user[i].user_id);
                    user.tenant = string(this._conferences[conf_id].user[i].tenant || this._tenant);
        ...
        getCallInfo: function(call_id) {
            ...
            if (this._sessionBundleTable[sessionBundleId]) {
                ...
                try {
                    callInfo.sharedObject = JSON.parse(this._sessionBundleTable[sessionBundleId].sharedObjectJson);
                } catch (e) {
                }
                callInfo.direction = this._sessionBundleTable[sessionBundleId].direction;
                callInfo.microphoneMuted = this._sessionBundleTable[sessionBundleId].microphoneMuted;
                callInfo.cameraMuted = this._sessionBundleTable[sessionBundleId].cameraMuted;
                for (var i = 0; i < this._sessionBundleTable[sessionBundleId].sessionIdArray.length; i++) {
                    var session = this._phone.getCallSession(this._sessionBundleTable[sessionBundleId].sessionIdArray[i]);
                    if (session) {
                        ...
                        } else if (session.status !== "incoming" && session.status !== "answering") {
                ...
                if (callInfo.status === Constants.CALL_STATUS_TERMINATED && !this._sessionBundleTable[sessionBundleId].autoTerminate) {
                    callInfo.status = Constants.CALL_STATUS_TALKING;
                }
            }
            if (callInfo.target.conf_id) {
                var users = this._conferences[callInfo.target.conf_id] && this._conferences[callInfo.target.conf_id].user || [];
                for (var i = 0; i < users.length; i++) {
                    ...
                    if (video_conf_status > 0) {
                        ...
                        if (!streamTables[key]) {
                            streamTables[key] = {
                                buddy: buddy,
                                status: video_conf_status === this.VIDEO_CONF_VIDEO ? Constants.STREAM_STATUS_DIALING : Constants.STREAM_STATUS_TALKING,
                                streamTable: {}
                            };
                        }
                    }
        ...
        _rpcCall: function(method, params, funcOK, funcError) { // in funcOK and funcError, "this" is the same this in _rpcCall
            if (this._rpc) {
                this._rpc.call(method, params,
                    function(result, obj) { // obj is this in _rpcCall
                        if (funcOK) {
                            funcOK.call(obj, result);
                        }
                    },
                    function(error, obj) { // obj is this in _rpcCall
                        obj._logger.log("info", "_rpcCall failed (method: " + method + ", code: " + string(error.code) + ", message: " + string(error.message) + ")");
                        if (funcError) {
                            funcError.call(obj, error);
                        }
                    },
                    this); // this will be returned as "obj" in callback functions
            } else {
                this._logger.log("info", "_rpcCall failed (method: " + method + ", message: RPC not opened)");
                if (funcError) {
                    funcError({ code: Brekeke.net.ERROR_NOT_OPENED, message: "RPC not opened" });
                }
            }
        },
        _rpcNotify: function(method, params, funcError) {
            if (this._rpc) {
                ...
                this._logger.log("info", "_rpcNotify failed (method: " + method + ", message: RPC not opened)");
                if (funcError) {
                    funcError({ code: Brekeke.net.ERROR_NOT_OPENED, message: "RPC not opened" });
                }
            }
        ...
        _forcedSignOut: function(code, message) {
            this._logger.log("warn", "_forcedSignOut code: " + code + ", message: " + message);
        ...
        _signedOut: function() {
            ...
            if (this._phone) {
                ...
                try {
                    ...
                    this._logger.log("warn", e.message + " at finalizePhone" + "\n" + e.stack + "\n");
                }
                this._phone = null;
            }
            this._phoneProperties = {};
        ...
        _signInSuccess: function() {
            ...
            // start ping timer
            this._pingTimer = setTimeout(this._byThis(this._keepAlive), this.CHECK_ALIVE_INTERVAL);
        ...
        _signInStartRpc: function(password) {
            ...
            // start json rpc
            var prms = "?tenant=" + encodeURIComponent(this._tenant) + "&user=" + encodeURIComponent(this._user_id);
            if (password) {
            ...
            try {
                ...
                this._logger.log("warn", e.message + " at _signInStartRpc" + "\n" + e.stack + "\n");
            }
            if (!this._rpc) {
        ...
        _signInActionStatusNG: function(args) {
            ...
            if (this._signInStatus === 2) {
                if (args.nonce) {
                    ...
                    var passwordHashedWithNonce = CryptoJS.MD5(this._user_id + ":" + args.nonce + ":" + hashedPassword).toString();
        ...
        _signInGetPropertiesOK: function(result) {
            if (this._signInStatus === 2) {
                ...
                if (this.getConfigProperties().buddy_mode === Constants.BUDDY_MODE_MANUAL ||
                ...
                if (buddy_mode === Constants.BUDDY_MODE_AUTO) {
                    ...
                    if (result && result.user && result.user.length) {
                        ...
                        for (var i = 0; i < result.user.length; i++) {
                            ...
                            if (pbxUser.user_id === this._user_id) { // except myself
                                continue;
                            }
                    ...
                    if (result && result.user && result.user.length) {
                        for (var i = 0; i < result.user.length; i++) {
                            ...
                            for (var j = 0; j < this._buddylist.user.length; j++) {
                                ...
                                if (buddy.user_id === pbxUser.user_id && buddy.tenant === this._tenant) {
                                    exists = true;
                                    break;
                                }
                            }
                        ...
                        for (var i = this._buddylist.user.length - 1; i >= 0; i--) {
                            var buddy = this._buddylist.user[i];
                            if (buddy.user_id || buddy.user_id === "") { // user
                                var exists = false;
        ...
        _getPhoneProperties: function() {
            this._phoneRegisterTimer = setTimeout(this._byThis(function() {
                this._logger.log("warn", "Phone register timeout");
                this._phoneStatusChanged(false, Errors.WEBRTC_TEMPORARILY_UNAVAILABLE, "Phone register timeout");
        ...
        _getPhonePropertiesOK: function(result) {
            ...
            if (this._phoneProperties.extra_headers) {
                try {
                    if (this._phoneProperties.extra_headers.charAt(0) !== '[') {
                        this._phoneProperties.extraHeaderArray = JSON.parse('["' + this._phoneProperties.extra_headers + '"]');
                    } else {
                        this._phoneProperties.extraHeaderArray = JSON.parse(this._phoneProperties.extra_headers);
                    }
                } catch(e) {
            ...
            if (this._phoneProperties.pnumber === "") {
                ...
                this._phoneStatusChanged(false, Errors.WEBRTC_PERMANENTLY_UNAVAILABLE, "Empty pnumber");
                return;
            }
            if (!this._phoneProperties.password && !this._phoneProperties.authorization) {
            ...
            try {
                this._phone.initPhone({
                        ...
                        ws_servers: this._useHttps ? ("wss://" + sip_host + ":" + this._phoneProperties.sip_wss_port) : ("ws://" + sip_host + ":" + this._phoneProperties.sip_ws_port),
                        display_name: "",
                        authorization_user: "",
                        register: true,
                    ...
                    {
                        registratorExtraHeaders: this._phoneProperties.authorization ? ["Authorization: " + this._phoneProperties.authorization] : null,
                        iceOnlyVideo: true,
                        userAgent: this._phoneProperties.user_agent
                    });
            } catch(e) {
                this._logger.log("warn", e.message + " at InitPhone" + "\n" + e.stack + "\n");
                
                this._phoneStatusChanged(false, Errors.WEBRTC_TEMPORARILY_UNAVAILABLE, e.message + " at InitPhone");
            }
        },
        _getPhonePropertiesNG: function(error) {
            this._logger.log("warn", error.message + " at GetPhoneProperties");
            
            this._phoneStatusChanged(false, Errors.WEBRTC_TEMPORARILY_UNAVAILABLE, error.message + " at GetPhoneProperties");
        },
        ...
        _sendStatus: function(funcOK, funcError) {
            ...
            this._rpcCall("ChangeStatus",
                ...
                function(result) {
                    if (result) {
                        this._download_keys[JSON.stringify({ tenant: this._tenant, user_id: this._user_id })] = result.dlk;
                    }
                    if (funcOK) {
                        var ev = {
                        };
        ...
        _recvNotifyStatus: function(args) {
            ...
            if (this._buddylist && this._buddylist.user && this._buddylist.user.length) {
                for (var i = 0; i < this._buddylist.user.length; i++) {
                    if (this._buddylist.user[i].user_id === sender_user_id && (this._buddylist.user[i].tenant || this._tenant) === sender_tenant) {
                        user = this._buddylist.user[i];
                    }
                }
            }
            if (user === null) { // nonbuddy
                isNonbuddy = true;
                if (this._nonbuddylist && this._nonbuddylist.user && this._nonbuddylist.user.length) {
                    for (var i = 0; i < this._nonbuddylist.user.length; i++) {
                        if (this._nonbuddylist.user[i].user_id === sender_user_id && (this._nonbuddylist.user[i].tenant || this._tenant) === sender_tenant) {
                            ...
                            if (result && result.user && result.user.length) {
                                for (var i = 0; i < result.user.length; i++) {
                                    ...
                                    if (!buddyExists) {
                                        ...
                                        userBuddy.block_settings = {};
                                        this._buddylist.user.push(userBuddy);
                                        if (userBuddy.user_id === sender_user_id && userBuddy._tenant === sender_tenant) {
                                            user = userBuddy;
                                            isNonbuddy = false;
                                        }
                                        modified = true;
        ...
        _recvNotifyStatus2: function(args, user, isNonbuddy) {
            var sender_tenant = string(args.sender.tenant || this._tenant);
            var sender_user_id = string(args.sender.user_id);
            var key = JSON.stringify({ tenant: sender_tenant, user_id: sender_user_id });
            ...
            // modify name
            if (user.name !== args.client_param.name && args.client_param.name !== null) {
        ...
        _getProfileImageUrl: function(tenant, user_id) {
            var key = JSON.stringify({ tenant: tenant, user_id: user_id });
            return (this._useHttps ? "https://" : "http://") + this._host + "/" + this._path + "/image?ACTION=DOWNLOAD&tenant=" + encodeURIComponent(tenant) + "&user=" + encodeURIComponent(user_id) + (this._download_keys[key] ? ("&dlk=" + encodeURIComponent(this._download_keys[key])) : "");
        },
        _recvText: function(args) {
            ...
            case Constants.CTYPE_FILE_REQUEST:
                ...
                if (fileProps.target && !(fileProps.target.tenant === this._tenant && fileProps.target.user_id === this._user_id)) {
                    ...
                    // new file info
                    this._newFileInfo(file_id, target, false, Constants.FILE_STATUS_UNACCEPTED, name, size, null, null);
                ...
                break;
            default:
                this._logger.log("warn", "Invalid argument: _recvText ctype=" + args.ctype);
                break;
            }
        },
        ...
        _invitedToConference: function(args) {
            ...
            this._conferences[conf_id].created_time = stringifyTstamp(args.created_tstamp);
            ...
            this._conferences[conf_id].invite_properties = {
                invisible: Boolean(args.join_properties && args.join_properties.invisible),
                webchatfromguest: args.join_properties && args.join_properties.webchatfromguest || null
            };
        ...
        _conferenceMemberChanged: function(args) {
            ...
            if (args.user && args.user.length) {
                for (var i = 0; i < args.user.length; i++) {
                    ...
                    users.push(user);
                    if (args.user[i].tenant === this._tenant && args.user[i].user_id === this._user_id) {
            ...
            if (conf_status === Constants.CONF_STATUS_INVITED) {
                ...
                this._conferences[conf_id].user.push({
                    ...
                    name: this._conferences[conf_id].from.user_name || this._conferences[conf_id].from.user_id,
                    conf_status: Constants.CONF_STATUS_JOINED
                });
            }
            
            // update assigned
            if (this._conferences[conf_id].invite_properties.webchatfromguest && !this._conferences[conf_id].assigned.user_id) {
                for (var i = 0; i < this._conferences[conf_id].user.length; i++) {
                    if (this._conferences[conf_id].user[i].conf_status === Constants.CONF_STATUS_JOINED &&
                        this._conferences[conf_id].user[i].user_id !== this._conferences[conf_id].creator.user_id) {
                        this._conferences[conf_id].assigned.tenant = this._conferences[conf_id].user[i].tenant;
                        this._conferences[conf_id].assigned.user_id = this._conferences[conf_id].user[i].user_id;
                        break;
                    }
                }
            }
        ...
        _newFileInfo: function(file_id, target, isUpload, status, name, size, form, formData) {
            // create file info
        ...
        _recvRecvFile: function(args) {
            if (!args.client_param || !args.client_param.response || !args.client_param.file_id) {
            ...
            if (fileInfo) {
                ...
                case Constants.CTYPE_FILE_ACCEPT:
                    // start upload post
                    setTimeout(this._byThis(function() { // wait to start sync on server
                        var fileInfo = this._fileInfos[file_id];
                        if (fileInfo) {
                            if (fileInfo._formData) { // FormData enabled
                                // upload with XHR + FormData
                                var fd = fileInfo._formData;
                                var xhr = new XMLHttpRequest();
                                xhr.open("POST", (this._useHttps ? "https://" : "http://") + this._host + "/" + this._path + "/file?ACTION=UPLOAD&SUBACTION=TRANS&TRANSFER_ID=" + encodeURIComponent(file_id) + "&tenant=" + encodeURIComponent(this._tenant) + "&user=" + encodeURIComponent(this._user_id), true);
                                xhr.upload.onprogress = this._byThis(function(evt) {
                                    if (evt.lengthComputable) {
                                        var progress = Math.min(99, Math.round(evt.loaded * 100 / evt.total));
                                        var progressTime = new Date().getTime();
                                        if (fileInfo.progress !== progress && fileInfo._lastProgressTime + 1000 <= progressTime) {
                                            this._changeFileInfo(file_id, Constants.FILE_STATUS_TRANSFERRING, progress);
                                ...
                                xhr.onload = this._byThis(function(evt) {
                                    this._terminateFileInfo(file_id, xhr.status === 200 ? Constants.FILE_STATUS_COMPLETED : Constants.FILE_STATUS_ERROR);
                                });
                                xhr.send(fd);
                                fileInfo._xhr = xhr;
                                // raise fileInfoChanged event
                                this._changeFileInfo(file_id, Constants.FILE_STATUS_TRANSFERRING, 0);
                            } else { // FormData disabled (IE9)
                                // upload with form
                                fileInfo._form.action = (this._useHttps ? "https://" : "http://") + this._host + "/" + this._path + "/file?ACTION=UPLOAD&SUBACTION=TRANS&TRANSFER_ID=" + encodeURIComponent(file_id) + "&tenant=" + encodeURIComponent(this._tenant) + "&user=" + encodeURIComponent(this._user_id);
                ...
                case Constants.CTYPE_FILE_REJECT:
                    this._rpcCall("StopFileTransfer",
                        ...
                        function(error) {
                            this._logger.log("warn", "Failed to stop file transfer: " + string(error.message));
                            if (fileInfo._xhr) {
                                fileInfo._xhr.upload.onabort = this._byThis(function(evt) {
                                    this._terminateFileInfo(file_id, Constants.FILE_STATUS_ERROR);
                                });
        ...
        _recvNotifyFileTransfer: function(args) {
            if (!args.client_param || !args.client_param.status || !args.client_param.file_id) {
        ...
        _sessionCreated: function(e) {
            ...
            if (callSession.status === "dialing") {
                ...
                if (this._makeCallStatus[sessionBundleId]) {
                    if (this._sessionBundleTable[sessionBundleId]) {
                        ...
                        if (callSession.option.mediaConstraints) {
                            sessionBundle.mediaConstraints = {
                                audio: callSession.option.mediaConstraints.audio || sessionBundle.mediaConstraints.audio,
                                video: callSession.option.mediaConstraints.video || sessionBundle.mediaConstraints.video
                            };
                        }
                        sessionBundle.sharedObjectJson = callSession.option.sharedObjectJson || "{}";
                        sessionBundle.singleSession = Boolean(callSession.option.singleSession);
                        ...
                        sessionBundle = {
                            ...
                            mediaConstraints: callSession.option.mediaConstraints || { audio: false, video: false },
                    ...
                    this._logger.log("warn", "Outgoing session without makeCall: " + sessionBundleId);
                    callSession.hangUp();
                }
            } else {
                // incoming call
                ...
                var tenant = null;
                var user_id = null;
                if (this._phoneProperties.mode_uc_call === 1 || (this._phoneProperties.mode_uc_call === 2 && !callSession.mediaConstraints.audio)) {
                ...
                if (user_id !== null) {
                    ...
                    for (var bundleId in this._sessionBundleTable) {
                        if (this._sessionBundleTable[bundleId].target.tenant === tenant && this._sessionBundleTable[bundleId].target.user_id === user_id) {
                    ...
                    loop_get_conf_id: for (var id in this._conferenceCalls) {
                        ...
                        if (conf) {
                            for (var i = 0; i < conf.user.length; i++) {
                                ...
                                if (string(user.tenant || this._tenant) === tenant && string(user.user_id) === user_id && int(user.video_conf_status) === this.VIDEO_CONF_VIDEO) {
                                    for (var bundleId in this._sessionBundleTable) {
                                        var bundle = this._sessionBundleTable[bundleId];
                                        if (bundle.target.conf_id === id) {
                                            var established = false;
                                            for (var j = 0; j < bundle.sessionIdArray.length; j++) {
                                                var sess = this._phone.getCallSession(bundle.sessionIdArray[j]);
                                                if (sess && string(sess.option.tenant) === tenant && string(sess.option.user_id) === user_id) {
                                                    established = true;
                                                    break;
                                                }
                                            }
                ...
                if (sessionBundleId === null) {
                    ...
                    if (conf_id !== null) {
                        ...
                        if (this._receivedSharedObjectJsonTable[key]) {
                            if (new Date().getTime() - this._receivedSharedObjectJsonTable[key].timestamp <= 10000) {
                                sessionBundle.sharedObjectJson = this._receivedSharedObjectJsonTable[key].json;
                            }
                    ...
                    if (sessionBundle.autoAnswer) {
                        var audioConstraints = callSession.mediaConstraints.audio ? sessionBundle.mediaConstraints.audio : false;
                        var videoConstraints = callSession.mediaConstraints.video ? sessionBundle.mediaConstraints.video : false;
                        if (audioConstraints || videoConstraints) {
                            callSession.answer({
                                audio: audioConstraints,
                                video: videoConstraints
        ...
        _callInfoChanged: function(sessionBundleId) {
            ...
            if (!terminated) {
                ...
                if (callInfo.target.conf_id) {
                    if (this._phoneProperties.mode_uc_call === 1 || this._phoneProperties.mode_uc_call === 2) {
        ...
        _recvCustomClientEvent: function(args) {
            ...
            if (this._eventListeners.receivedCustomClientEvent && this._signInStatus === 3) {
                var ev = {
                    tenant: sender_tenant,
                    user_id: sender_user_id,
        ...
        RecvClientEvent: function(args) {
            ...
            case "Dummy":
                break;
            default:
                this._logger.log("warn", "Not implemented: ChatClient.RecvClientEvent client_method=" + args.client_method);
                break;
            }
        },
