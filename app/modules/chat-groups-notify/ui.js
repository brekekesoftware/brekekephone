...
const Notify = (p) => (
  <View style={st.notify}>
    <View style={st.notifyInfo}>
      ...
      <Text style={st.notifySubtitle}>
        by {p.inviter}
      </Text>
    </View>
    <Button style={st.reject}
      onPress={p.reject}>
      <Text style={st.rejectIcon}>
        icon_x
      </Text>
    </Button>
    <Button style={st.accept}
      onPress={p.accept}>
      <Text style={st.acceptIcon}>
        icon_check
      </Text>
    </Button>
  </View>
...
const ChatGroupsNotify = (p) => (
  p.groups.map((group) => <Notify
    key={group} {...p.formatGroup(group)}
