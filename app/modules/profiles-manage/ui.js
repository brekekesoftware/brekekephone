...
import {StyleSheet, View, ScrollView as Scroll, TouchableOpacity as Button, Text, Platform } from 'react-native'
...
const Navbar = (p) => (
  <View style={st.navbar}>
    <Text style={st.navbarTitle}>
        {UserLanguage.getUserzMessage_s("com.brekeke.phone.app.modules.profiles-manage.PROFILES")}
    </Text>
...
const ProfileItem = (p) => (
    <View style={st.profile}>
        ...
        <Button style={st.profileAction}
                onPress={p.remove}>
            <Text style={st.dangerIcon}>
                icon_trash
            </Text>
        </Button>
        <Button style={st.profileAction}
...
const Profiles = (p) => p.ids.length ? (
  <Scroll style={st.profiles}>
    {p.ids.map((id) => <ProfileItem
      key={id} {...p.resolve(id)}
      remove={() => p.remove(id)}
      update={() => p.update(id)}
      signin={() => p.signin(id)}
