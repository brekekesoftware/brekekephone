...
import {StyleSheet, View, ScrollView, TouchableOpacity as Button, Text, TextInput, Switch} from 'react-native'
...
const Call = pure((p) => (<Fragment>
    <Divider>CALL</Divider>
    <View style={st.call}>
        <Text style={st.partyName}>
            {p.partyName || p.partyNumber}
        </Text>
        <Text style={st.partyNumber}>
            {p.partyNumber}
        </Text>
    </View>
</Fragment>))
...
const Match = pure((p) => {
    ...
    return <Button style={st.match}
            onPress={() => p.select(p.number)}>
        <View style={st.matchInfo}>
            <Text style={st.matchName}>
                { matchName }
            </Text>
            <Text style={st.matchNumber}>
                {p.number}
            </Text>
        </View>
        {p.talking ? (
...
const Matches = pure((p) => {
    return <ScrollView style={st.matches}>
        {p.ids.map((id) => <Match key={id}
                                  {...p.resolve(id)}
                                  select={p.select}
        />)}
    </ScrollView>
...
const CallTransferDial = (p) => p.call
    ? <Main {...p} />
    : <NoCall back={p.back} />
