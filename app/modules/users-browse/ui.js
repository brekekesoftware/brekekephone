...
import {StyleSheet, View, ScrollView, TouchableOpacity as Button, Text, Image, TextInput} from 'react-native'
...
const Navbar = pure((p) => (
    <View style={st.navbar}>
        <Text style={st.navbarTitle}>
            {UserLanguage.getUserzMessage_s("com.brekeke.phone.app.modules.users-browse.USERS")}
        </Text>
    </View>
))
...
const Search = pure((p) => (
    <View style={st.search}>
        <TextInput style={st.searchInput}
                   placeholder={UserLanguage.getUserzMessage_s("com.brekeke.phone.app.modules.users-browse.SEARCH")}
                   value={p.value}
...
const User = pure((p) => (
    <View style={st.user}>
        ...
        <Button style={st.userOpt} onPress={p.callVideo}>
            ...
            {p.callHolding && (
                <View style={st.userCallHolding} />
            )}
            {p.callTalking && (
                <View style={st.userCallTalking} />
            )}
            {p.callRinging && (
                <View style={st.userCallRinging} />
            )}
            {p.callCalling && (
                <View style={st.userCallCalling} />
            )}
        </Button>
        {p.chatEnabled && (
            <Button style={st.userOpt} onPress={p.chat}>
                ...
                {p.chatOffline && (
                    <View style={st.userChatOffline} />
                )}
                {p.chatOnline && (
                    <View style={st.userChatOnline} />
                )}
                {p.chatIdle && (
                    <View style={st.userChatIdle} />
                )}
                {p.chatBusy && (
                    <View style={st.userChatBusy} />
                )}
            </Button>
        )}
