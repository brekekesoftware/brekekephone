...
const Buddy = (p) => (
  <Button style={st.chat} onPress={p.select}>
    <View style={st.chatIcon}>
      <Image style={st.buddyAvatar}
        source={{uri: p.avatar}}
      />
      {p.offline && (
        <View style={st.chatOffline} />
      )}
      {p.online && (
        <View style={st.chatOnline} />
      )}
      {p.idle && (
        <View style={st.chatIdle} />
      )}
      {p.busy && (
        <View style={st.chatBusy} />
      )}
    </View>
      {( () => {
            if( p.name ) {
                return <Text style={st.chatName}>
                    {p.name}
                </Text>;
            }
            else{
...
const Group = (p) => (
  <Button style={st.chat} onPress={p.select}>
    <View style={st.chatIcon}>
      <Text style={st.groupIcon}>
        icon_users
      </Text>
    </View>
    <Text style={st.chatName}>
      {p.name}
    </Text>
    <Text style={st.chatChevronIcon}>
      icon_chevron_right
...
const ChatsRecent = (p) => (
  <View style={st.main}>
    <Navbar />
    <Button style={st.createGroup}
      onPress={p.createGroup}>
      <Text style={st.createGroupText}>
        Create Group
      </Text>
    </Button>
