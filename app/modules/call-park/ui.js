...
import {StyleSheet, View, ScrollView, TouchableOpacity as Button, Text} from 'react-native'
...
const Park = (p) => (
  <Button style={st.park} onPress={p.select}>
    {p.selected && <View
      style={st.parkSelected}
    />}
    <Text style={st.parkText}>
      {p.id}
    </Text>
  </Button>
)
...
const CallPark = (p) => (
  !p.call ? <NoCall back={p.back} />
    : !p.parks.length ? <NoPark back={p.back} />
      : <Main {...p} />
)
