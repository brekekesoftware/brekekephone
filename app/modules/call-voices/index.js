...
const mapGetter = (getter) => (state) => ({
  incomingCallIds: getter.runningCalls.idsByOrder(state).filter((id) =>
    isIncoming(getter.runningCalls.detailMapById(state)[id])
  ),
  outgoingCallIds: getter.runningCalls.idsByOrder(state).filter((id) =>
    isOutgoing(getter.runningCalls.detailMapById(state)[id])
  ),
  answeredCallIds: getter.runningCalls.idsByOrder(state).filter((id) =>
    isAnswered(getter.runningCalls.detailMapById(state)[id])
  ),
...
export default
createModelView(mapGetter, null)(View)
