...
import { orderBy, uniqBy } from '@/shared/lodash'
import type { UcMessageLog } from '#/brekekejs'
import { Constants } from '#/brekekejs/ucclient'
import { ListUsers } from '#/components/chat-list-users'
import { Field } from '#/components/field'
import { Layout } from '#/components/layout'
import { RnText } from '#/components/rn'
import type { ChatGroup, ChatMessage } from '#/stores/chat-store'
import type { UcUser } from '#/stores/contact-store'
import { ctx } from '#/stores/ctx'
import { intl } from '#/stores/intl'
import { arrToMap } from '#/utils/arr-to-map'
import { filterTextOnly, formatChatContent } from '#/utils/format-chat-content'

export const PageChatRecents = observer(() => {
  const getLastChat = (id: string) => {
    const chats = filterTextOnly(ctx.chat.getMessagesByThreadId(id))
    return chats.length ? chats[chats.length - 1] : ({} as ChatMessage)
  }
  const saveLastChatItem = async (
    arr: {
      id: string
      name: string
      text: string
  ...
  const webchatInactive = ctx.chat.groups.filter(
    gr => gr.webchat && gr.webchat.conf_status !== Constants.CONF_STATUS_JOINED,
  )
  ...
  recentGroups.push(
    ...groupIds.map(id => ({ ...getLastChat(id), threadId: id })),
  )
  ...
  recentUsers.push(
    ...threadIds.map(id => ({ ...getLastChat(id), threadId: id })),
  )
